...
class MAGNAKGlayer(nn.Module):
    ...
    def forward(self, graph: DGLGraph, ent_embed, rel_embed, drop_edge_ids=None):
        ...
        if drop_edge_ids is not None:
            attations[drop_edge_ids.to(torch.long)] = self.attention_mask_value
    ...
    def topk_attention(self, graph: DGLGraph):
        ...
        # graph.register_reduce_func(topk_attn_reduce_func) #目标节点在获得其他节点以及边的特征信息之后，通过累和函数计算出一个新的表示。
        # graph.register_message_func(send_edge_message)  #消息函数可以获得出发节点和目标节点的特征信息，描述了需要发给目标节点做下一步计算的信息。
