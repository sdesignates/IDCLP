...
# from HGAT.layernormalization import RMSLayerNorm as LayerNorm
from HGAT.layernormalization import STDLayerNorm as LayerNorm
...
class PositionwiseFeedForward(nn.Module):
    ...
    def init(self):
        gain = nn.init.calculate_gain('relu') # nn.init.calculate_gain(nonlinearity, param=None), Return the recommended gain value for the given nonlinearity function
        nn.init.xavier_normal_(self.w_1.weight, gain=gain) # gain=1.414
...
class MAGNALayer(nn.Module):
    ...
    def forward(self, graph: DGLGraph, features, drop_edge_ids=None):
        ...
        graph = graph.local_var() # any out-place mutation to the feature data will not reflect to this graph
        if self.layer_norm:
            h = self.graph_norm(features)  # Gaussian  distribution
        else:
            h = features
        if self.head_tail_shared:
            ...
            graph.ndata.update({'ft': feat, 'eh': eh, 'et': et})
        graph.apply_edges(fn.u_add_v('eh', 'et', 'e')) # .apply_edges: Update the features of the specified edges, .u_add_v: performing element-wise add between features of u and v
        ...
        rst = self.ppr_estimation(graph=graph) # PageRank
        rst = rst.flatten(1)
        rst = self.fc_out(rst)
    ...
    def ppr_estimation(self, graph: DGLGraph):
        ...
        for _ in range(self.hop_num):
            ...
            graph.update_all(fn.u_mul_e('h', 'a_temp', 'm'), fn.sum('m', 'h')) # aggregates messages by sum.
    ...
    def topk_attention(self, graph: DGLGraph):
        ...
        def topk_attn_reduce_func(nodes):
            topk = self.top_k
            attentions = nodes.mailbox['m_e']  # .mailbox: Return a view of the messages received.
            neighbor_num = attentions.shape[1]
            if topk > neighbor_num:
                topk = neighbor_num
            topk_atts, _ = torch.topk(attentions, k=topk, dim=1)
            kth_attn_value = topk_atts[:, topk-1]  # the top k-th value
            return {'kth_e': kth_attn_value}

        graph.register_reduce_func(topk_attn_reduce_func) # Register global message reduce function. Once registered, ``func`` will be used as the default
