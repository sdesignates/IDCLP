...
import torch
from HGAT.MAGNA_KGConv import MAGNAKGlayer
...
class MAGNAKGEncoder(nn.Module):
    def __init__(self,
        ...
        self.trans_layers.append(
            MAGNAKGlayer(in_ent_feats=in_ent_dim, num_heads=num_heads, in_rel_feats=in_rel_dim, out_feats=hidden_dim,
                         feat_drop=feat_drop, hop_num=self.hop_num, top_k=self.top_k, alpha=self.alpha,
                         attn_drop=attn_drop, topk_type=topk_type, negative_slope=negative_slope, input_drop=input_drop,
                         args=args)
        )
        for l in range(1, num_layers):
            self.trans_layers.append(
                MAGNAKGlayer(in_ent_feats=hidden_dim,  num_heads=num_heads, in_rel_feats=in_rel_dim,out_feats=hidden_dim,
                             hop_num=self.hop_num, top_k=self.top_k, alpha=self.alpha, feat_drop=feat_drop,
                             topk_type=topk_type, attn_drop=attn_drop, negative_slope=negative_slope, input_drop=input_drop,
                             args=args)
            )

    def forward(self, graph: DGLGraph, entity_embedder, rel_embedder):
        h = entity_embedder
        h_r = rel_embedder
        ...
        for l in range(self.num_layers):
            h = self.trans_layers[l](graph, h, h_r, drop_edges_ids)
            feats.append(h)
        return feats
